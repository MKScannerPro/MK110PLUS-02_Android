...
public class AboutActivity extends BaseActivity<ActivityAboutBinding> {
    ...
    public void onFeedbackLog(View view) {
        if (isWindowLocked())
            return;
        File trackerLog = new File(RemoteMainWithMetering02Activity.PATH_LOGCAT + File.separator + "MKRemoteGW02.txt");
        File trackerLogBak = new File(RemoteMainWithMetering02Activity.PATH_LOGCAT + File.separator + "MKRemoteGW02.txt.bak");
        File trackerCrashLog = new File(RemoteMainWithMetering02Activity.PATH_LOGCAT + File.separator + "crash_log.txt");
