...
import com.moko.mkremotegw02.activity.add.DeviceScanner02Activity;
...
public class RemoteMainWithMetering02Activity extends BaseActivity<ActivityMainRemoteWithMetering02Binding> implements BaseQuickAdapter.OnItemClickListener, BaseQuickAdapter.OnItemLongClickListener {
    ...
    protected void onCreate() {
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                PATH_LOGCAT = getExternalFilesDir(null).getAbsolutePath() + File.separator + (BuildConfig.IS_LIBRARY ? "MKScannerPro" : "MKRemoteGW02");
            } else {
                PATH_LOGCAT = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + (BuildConfig.IS_LIBRARY ? "MKScannerPro" : "MKRemoteGW02");
            }
        } else {
            // 如果SD卡不存在，就保存到本应用的目录下
            PATH_LOGCAT = getFilesDir().getAbsolutePath() + File.separator + (BuildConfig.IS_LIBRARY ? "MKScannerPro" : "MKRemoteGW02");
        }
        ...
        devices = DBTools02.getInstance(this).selectAllDevice();
        adapter = new Device02Adapter();
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                startActivityForResult(new Intent(this, SetAppMQTT02Activity.class), AppConstants.REQUEST_CODE_MQTT_CONFIG_APP);
            }
            // 读取stacktrace信息
        ...
        if (!devices.isEmpty()) {
            for (MokoDevice device : devices) {
                if (device.mac.equals(event.getMac())) {
                    device.name = DBTools02.getInstance(this).selectDevice(device.mac).name;
                    break;
                }
            }
        }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (getIntent().getExtras() != null) {
            ...
            if (ModifyName02Activity.TAG.equals(from)
                    || DeviceSetting02Activity.TAG.equals(from)) {
                devices.clear();
                devices.addAll(DBTools02.getInstance(this).selectAllDevice());
            ...
            if (ModifySettings02Activity.TAG.equals(from)) {
                if (!TextUtils.isEmpty(mac)) {
                    MokoDevice mokoDevice = DBTools02.getInstance(this).selectDevice(mac);
                    for (final MokoDevice device : devices) {
                        if (mac.equals(device.mac)) {
...
//                return;
//            }
        startActivity(new Intent(this, DeviceScanner02Activity.class));
//        } else {
...
//        }
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                        ...
                        if (commonResp.code != 200) {
                            com.moko.lib.scannerui.utils.ToastUtils.showToast(RemoteMainWithMetering02Activity.this, commonResp.msg);
                            LoginDialog dialog = new LoginDialog();
                        ...
                        IoTDMSPUtils.setStringValue(RemoteMainWithMetering02Activity.this, IoTDMConstants.EXTRA_KEY_LOGIN_ACCOUNT, account);
                        IoTDMSPUtils.setStringValue(RemoteMainWithMetering02Activity.this, IoTDMConstants.EXTRA_KEY_LOGIN_PASSWORD, password);
                        IoTDMSPUtils.setIntValue(RemoteMainWithMetering02Activity.this, IoTDMConstants.EXTRA_KEY_LOGIN_ENV, envValue);
                        Intent intent = new Intent(RemoteMainWithMetering02Activity.this, SyncDeviceActivity.class);
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        ...
        Intent i = new Intent(this, DeviceDetail02Activity.class);
