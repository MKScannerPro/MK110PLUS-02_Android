...
import com.moko.mkremotegw02.databinding.ActivityMqttDevice02Binding;
...
public class MqttSettings02Activity extends BaseActivity<ActivityMqttDevice02Binding> implements RadioGroup.OnCheckedChangeListener {
    ...
    @Override
    protected ActivityMqttDevice02Binding getViewBinding() {
        return ActivityMqttDevice02Binding.inflate(getLayoutInflater());
    }
...
//                    row17.createCell(1).setCellValue("");
                ...
                runOnUiThread(() -> {
                    ...
                    if (isFileError) {
                        isFileError = false;
                        ToastUtils.showToast(MqttSettings02Activity.this, "Export error!");
                        return;
                    }
                    ToastUtils.showToast(MqttSettings02Activity.this, "Export success!");
                    Utils.sendEmail(MqttSettings02Activity.this, "", "", "Settings for Device", "Choose Email Client", expertFile);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (rows < 18 || columns < 3) {
                                runOnUiThread(() -> {
                                    dismissLoadingProgressDialog();
                                    ToastUtils.showToast(MqttSettings02Activity.this, "Please select the correct file!");
                                });
                                return;
                            }
                            ...
                            runOnUiThread(() -> {
                                dismissLoadingProgressDialog();
                                if (isFileError) {
                                    ToastUtils.showToast(MqttSettings02Activity.this, "Import failed!");
                                    return;
                                }
                                ToastUtils.showToast(MqttSettings02Activity.this, "Import success!");
                                initData();
                            });
                        } catch (Exception e) {
                            e.printStackTrace();
